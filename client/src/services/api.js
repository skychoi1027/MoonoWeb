 ...
 * @returns {Promise} 완성된 텍스트
 */
export const completeText = async (prompt, maxTokens = 500) => {
  try {
...
export const getData = async () => {
  try {
    ...
    console.error('Get Data 오류:', error);
    throw error;
  }
};
 ...
 * 데이터 생성
 */
export const createData = async (title, content, metadata = {}) => {
  try {
    const response = await fetch(`${API_BASE_URL}/api/data`, {
      method: 'POST',
      headers: {
    ...
    console.error('Create Data 오류:', error);
    throw error;
  }
};
